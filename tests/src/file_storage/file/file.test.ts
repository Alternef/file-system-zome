...
test('create file, update it, read it and delete in cascade', async () => {
    await runScenarioWithTwoAgents(async (scenario, alice, bob) => {
        ...
        const readChunksOutput = await getFileChunks(bob.cells[0], original_action_hash);
        assert.equal(readChunksOutput.length, 0);

        // Check if the second update's metadata is deleted
        const readSecondOutput = await getFileMetadata(bob.cells[0], secondUpdateRecords.file_metadata.signed_action.hashed.hash);
        assert.isNull(readSecondOutput);

		// Check if the second update's chunks are deleted
		const readSecondChunksOutput = await getFileChunks(bob.cells[0], secondUpdateRecords.file_metadata.signed_action.hashed.hash);
...
test('create file with empty name and check path standardization', async () => {
	await runScenarioWithTwoAgents(async (scenario, alice, bob) => {
		...
		await expect(createFile(alice.cells[0], sampleFileInput("/", "")))
		.rejects.toThrow();

		// Test case 2: Path with forbidden characters
		await expect(createFile(alice.cells[0], sampleFileInput("/?.34", "test.txt")))
		.rejects.toThrow();

		// Test case 3: Standardize path with '/'
		const record1 = await createFile(alice.cells[0], sampleFileInput("/", "test.txt"));
		assert.ok(record1);
		const decodedOutput1 = decodeOutputs([record1.file_metadata])[0] as FileMetadata;

		// Test case 4: Standardize path with '\'
		const record2 = await createFile(alice.cells[0], sampleFileInput("\\", "test2.txt"));
		assert.ok(record2);
		const decodedOutput2 = decodeOutputs([record2.file_metadata])[0] as FileMetadata;
